...
parser.add_argument(
    "--json_hr", type=int, default=8, metavar="S", help="json_hr (default: 7)"
)
parser.add_argument(
    "--json_tsetp",
    type=int,
...
json_path = (
    ...
    / "ecr"
    / "sz"
    / "scenario_shenzhen_downtown_west.json"
)
scenario = Scenario(
    json_file=json_path,
    sd=args.seed,
    ...
    for i_episode in epochs:
        ...
        for step in range(T):
            ...
            obs, paxreward, done, info = env.pax_step(
                CPLEXPATH=args.cplexpath, PATH="scenario_she"
            )
            ...
            rebAction = solveLCP(env, "scenario_she", desiredAcc, args.cplexpath)
        ...
        if episode_reward >= best_reward:
            model.save_checkpoint(path=f"./saved_files/ckpt/she/graph_rl.pth")
        ...
        model.log(log, path=f"./{args.directory}/rl_logs/she/graph_rl.pth")
else:
    if args.algo == "rl":
        # Load pre-trained model
        model.load_checkpoint(path=f"./{args.directory}/ckpt/she/graph_rl.pth")
    if args.algo == "oracle":
    ...
    for episode in epochs:
        ...
        while not done:
            if args.algo == "oracle":
                ...
                obs, paxreward, done, info = env.pax_step(
                    CPLEXPATH=args.cplexpath, PATH="scenario_she_test"
                )
                episode_reward += paxreward
                if args.algo == "rl":
                ...
                rebAction = solveLCP(
                    env, "scenario_she_test", desiredAcc, args.cplexpath
                )
                # Take action in environment
                new_obs, rebreward, done, info = env.reb_step(rebAction)
                episode_reward += rebreward
