...
class MPC:
    ...
    def MPC_exact(self, modPath=None, MPCPath=None, forecast=False):
                ...
                (
                    ...
                    max(
                        [
                            0,
                            np.random.normal(
                                self.env.scenario.demand_input[i, j][tt], 10
                            )
                            if tt > t
                            else self.env.demand[i, j][tt],
                        ]
                    ),
                    self.env.demandTime[i, j][tt],
                    self.env.price[i, j][tt],
                )
                for i, j in self.env.demand
                for tt in range(t, t + self.T)
                if tt in self.env.demand[i, j]
                and self.env.scenario.demand_input[i, j][tt] > 1e-3
            ]
        else:
            demandAttr = [
                (
                    ...
                    j,
                    tt,
                    self.env.demand[i, j][tt],
                    self.env.demandTime[i, j][tt],
                    self.env.price[i, j][tt],
                )
                for i, j in self.env.demand
                for tt in range(t, t + self.T)
                if tt in self.env.demand[i, j] and self.env.demand[i, j][tt] > 1e-3
            ]
        accTuple = [(n, self.env.acc[n][t]) for n in self.env.acc]
        daccTuple = [
            (n, tt, self.env.dacc[n][tt])
            for n in self.env.acc
            for tt in range(t, t + self.T)
        ]
        ...
        MPCPath = (
            os.getcwd().replace("\\", "/") + "/saved_files/cplex_logs/mpc/"
            if MPCPath is None
            else MPCPath
        )
        ...
        paxAction = [
            paxFlow[i, j] if (i, j) in paxFlow else 0 for i, j in self.env.edges
        ]
        rebAction = [
            rebFlow[i, j] if (i, j) in rebFlow else 0 for i, j in self.env.edges
        ]
