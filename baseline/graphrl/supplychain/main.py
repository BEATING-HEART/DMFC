...
from src.envs.scim_env import MyNetwork, MySCIM
...
path = Path(__file__).parents[3] / "data" / "scim" / "scim.json"
with open(path, "r") as f:
    data = json.load(f)
data = data[args.dataset]


network = MyNetwork(data=data)
env = MySCIM(network)


if args.algo == "rl":
    # Initialize agent
    parser = GNNParser(env, edge_list=env.edge_list)
    actor = Actor(
    ...
    for i_episode in epochs:
        ...
        if episode_reward >= best_reward:
            model.save_checkpoint(
                path=f"./saved_files/ckpt/{args.dataset}/graph_rl.pth"
            )
            best_reward = episode_reward
        # Log KPIs
        log["train_reward"].append(episode_reward)
        model.log(log, path=f"./{args.directory}/rl_logs/{args.dataset}/graph_rl.pth")
else:
    np.random.seed(args.seed)
    if args.algo == "rl":
        # Load pre-trained model
        model.load_checkpoint(
            path=f"{Path(__file__).parent}/{args.directory}/pretrained/{args.dataset}/graph_rl.pth"
        )
        ...
        while not done:
            if args.algo == "oracle":
                ...
                if args.algo == "rl":
                    ...
                    # logger.warning(f"rl_prod: {prod}")
                    # logger.warning(f"rl_ship: {ship}")
