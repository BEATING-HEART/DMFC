...
G = nx.DiGraph()
edge_list = [(0, i) for i in range(1, 11)]
...
tf = 30
factory_nodes = [0]
warehouse_nodes = list(range(1, 11))
production_time = 1
product_prices = [15]
production_costs = [5]
storage_capacities = [100] + [15] * 10
storage_costs = [1] + [2] * 10
edge_costs = [0.3] * 10
edge_time = [1] * 10
network = Network(
    G=G,
...
if not args.test:
    ...
    for i_episode in epochs:
        ...
        for step in range(T):
            ...
            # solve LCP
            action = solveLCP(
                env, ship, prod, CPLEXPATH=args.cplexpath, res_path="scim_1f10s"
            )
        ...
        if episode_reward >= best_reward:
            model.save_checkpoint(path=f"./saved_files/ckpt/1f10s/graph_rl.pth")
    ...
    if args.algo == "rl":
        ...
        model.load_checkpoint(
            path=f"{Path(__file__).parent}/{args.directory}/pretrained/1f10s/graph_rl.pth"
        )
