...
class GNNParser:
    ...
    def parse_obs(self, obs):
        ...
        t = self.env.time
        # Node features
        avail = (  # current available stock in all nodes
            torch.tensor([int(self.env.acc[t - 1][n]) for n in self.env.nodes]).view(
                1, 1, len(self.env.nodes)
            )
        )
        demand_pred = torch.tensor(  # demand prediction for T=3 look-ahead
            [obs[2][tt][n] for tt in range(t, t + 3) for n in self.env.nodes]
        ).view(1, 3, int(len(self.env.nodes)))
        store_inflow = torch.tensor(  # arriving flow in the next 3 timesteps
            [
                int(self.env.arrival_flow[tt][n])
                for tt in range(t - 1, t + self.T)
                for n in self.env.nodes
            ]
        ).view(1, 3, int(len(self.env.nodes)))
        prod_inflow = torch.tensor(  # completed prod in the next 3 timesteps
            [
                int(self.env.arrival_prod[tt][n])
                for tt in range(t - 1, t + self.T)
                for n in self.env.nodes
            ]
        ).view(1, 3, int(len(self.env.nodes)))
        ...
        e = (
            torch.cat(
                (
                    torch.tensor(
                        [obs[0].edges[e]["edge_time"] for e in self.edge_list * 2]
                    )
                    .view(1, edge_index.shape[1])
                    .float(),
                    torch.tensor(
                        [obs[0].edges[e]["edge_cost"] for e in self.edge_list * 2]
                    )
                    .view(1, edge_index.shape[1])
                    .float(),
                ),
...
class Actor(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        mu, sigma = (
            F.softplus(self.h_to_mu(x_pp[: self.num_factories])),
            F.softplus(self.h_to_sigma(x_pp[: self.num_factories])),
        )
