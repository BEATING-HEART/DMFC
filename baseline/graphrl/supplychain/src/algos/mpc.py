...
def MPC(t, env, T=10, dataPath=None, modPath=None, matchingPath=None, CPLEXPATH=None):
    datafile = (
        os.getcwd().replace("\\", "/")
        + "/saved_files/cplex_logs/mpc/"
        + f"data_{t}.dat"
    )
    storageCost = [
        (n, env.random_graph.nodes[n]["storage_cost"]) for n in env.random_graph
    ]
    productionCost = [
        (n, env.random_graph.nodes[n]["production_cost"]) for n in env.scenario.factory
    ]
    storageCapacity = [
        (n, env.random_graph.nodes[n]["storage_capacity"]) for n in env.random_graph
    ]
    ...
    demand = [
        (n, tt - t + 1, env.demand[tt][n])
        for tt in range(t, t + T)
        for n in env.scenario.warehouse
    ]
    edgeAttr = [
        (
            i,
            j,
            env.random_graph.edges[i, j]["time"],
            env.random_graph.edges[i, j]["cost"],
        )
        for i, j in env.random_graph.edges
    ]
    arrivalTuple = [
        (n, tt - t + 1, env.arrival_flow[tt][n])
        for tt in range(t, t + T)
        for n in env.scenario.warehouse
    ]
    expectedProdTuple = [
        (n, tt - t + 1, env.arrival_prod[tt][n])
        for tt in range(t, t + T)
        for n in env.scenario.factory
    ]
    resfile = (
        os.getcwd().replace("\\", "/") + "/saved_files/cplex_logs/mpc/" + f"res_{t}.dat"
    )
    modPath = (
        os.getcwd().replace("\\", "/") + "/src/cplex_mod/"
        if modPath is None
        else modPath
    )
    ...
    prod = {
        (i): production[i] if (i) in production else 0 for i in env.scenario.factory
    }
    action = (prod, ship)
    return action
