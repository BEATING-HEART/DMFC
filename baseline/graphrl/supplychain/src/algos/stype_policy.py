...
def s_type_policy(env, s_store, s_factory):
    ...
    for i, j in env.G.edges:
        if j in env.scenario.warehouse:
            desiredStoreOrder[(i, j)] = s_store - (
                env.acc[t - 1][j] + env.arrival_flow[t][j]
            )
    ...
    # otherwise, select store orders to maximize the minimum inventoy among all stores
        ...
        ratios = [
            desiredStoreOrder[0, j] / desiredStoreOrder_sum
            if j in env.scenario.warehouse
            else None
            for i, j in env.G.edges
        ]
        for i, key in enumerate(desiredStoreOrder.keys()):
            ship[key] = (env.acc[t - 1][key[0]] + env.arrival_prod[t][key[0]]) * ratios[
                i
            ]
    ...
    for factory in env.scenario.factory:
        av_factory[factory] = (
            env.acc[t - 1][factory]
            + env.arrival_prod[t][factory]
            - sum([ship[key] for key in ship])
        )
