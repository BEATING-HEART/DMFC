...
dvar float epsilon[region];

minimize(sum(e in edge)(rebFlow[e]*time[e]) + sum(i in region)(abs(epsilon[i])*1000000));
subject to
{
  forall(i in region)
    {
    sum(e in edge: e.i==i && e.i!=e.j) (rebFlow[<e.j, e.i>] - rebFlow[<e.i, e.j>]) >= desiredVehicles[i] - vehicles[i] + epsilon[i];
...
main {
  ...
  ofile.writeln("];")
  ofile.writeln("Optimal_Value=" + cplex.getObjValue());
  ofile.close();
}